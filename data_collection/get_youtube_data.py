...
if __name__ == "__main__":
    ...
    if not path.exists(SAVEFILE_NAME):
        ...
        final_results = pd.read_csv(
            SAVEFILE_NAME
        )  # continue with existing for nth crawl
        final_results = remove_redundancy(
    ...
    try:
        for search_query in search_queries:
            for i in range(10):  # just change this value for how many pages to run
                ...
                next_page_df = (
                    pd.DataFrame.from_dict(next_page_results)
                    .set_index("videoId")
                    .reset_index()
                )
                next_page_df[
                    "search_query"
                ] = search_query  # adding search query column value

                # add to existing data
                results_to_add = remove_redundancy(
                    final_results, next_page_df.itertuples(index=False)
                )
